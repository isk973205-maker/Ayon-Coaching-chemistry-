...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.ayancoaching.chemlab"
    minSdk = 24
    targetSdk = 36
    versionCode = 1
    versionName = "1.0"
...
tasks.register("copyDebugApk") {
    ...
    doLast {
        ...
        if (apkFile.exists()) {
            ...
            println("APK copied successfully to both .build-outputs and APK_DOWNLOAD! Size: ${apkFile.length()} bytes.")
        } else {
            error("Built APK file not found at: ${apkFile.absolutePath}")
        }
    }
}
